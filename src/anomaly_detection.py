...
class AnomalyDetector:
    ...
    @staticmethod
    def anomaly_detector(X_train, y_train, X_test, y_test_truth, mode):
        ...
        if mode=='M':
            ...
            # 1. Base model
            # -------------------------
            base_model = IsolationForest(n_estimators=400, max_samples=0.7, max_features=0.8, contamination=0.01,
                n_jobs=-1, random_state=42)

            # -------------------------
            # 2. Model fitting (with timing)
            # -------------------------
            start_fit = time.time()

            if mode == "tuned" and y_train is not None:

                param_grid = {"n_estimators": [300, 400, 600], "max_samples": [0.5, 0.7, 1.0],
                    "max_features": [0.6, 0.8, 1.0], "contamination": [0.005, 0.01, 0.02]}

                search = RandomizedSearchCV(estimator=base_model, param_distributions=param_grid, n_iter=20,
                    scoring="average_precision", cv=3, n_jobs=-1, random_state=42, verbose=0)
            ...
            y_pred_adjusted = (test_scores >= threshold).astype(int)
