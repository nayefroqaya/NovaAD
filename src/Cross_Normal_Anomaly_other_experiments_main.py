...
def main():
    ...
    SOURCE_DATASETS = ["HDFS","TH_1G"]        # example: ["BGL"], ["BGL", "TH_1G"]
    TARGET_DATASET = "BGL"          # example: "HDFS", "SP_150MB_ratio"
    ...
    # ============================================================
    # Select fraction of normal + anomaly target training blocks
    # ============================================================

    def select_target_fraction(target_train_df, fraction, seed=123):
        """
        Select the same fraction of normal and anomaly target TRAINING sequences.
        ...
        unique_anomaly_blocks = target_train_df[
            target_train_df["Label"] == "Anomaly"
        ]["Node_block_id"].unique()

        print(f"[INFO] Target normal blocks available: {len(unique_normal_blocks)}")
        print(f"[INFO] Target anomaly blocks available: {len(unique_anomaly_blocks)}")
        ...
        def sample_blocks(blocks):
            ...
            sample_size = max(1, int(round(len(blocks) * fraction)))
            sample_size = min(sample_size, len(blocks))
            return rng.choice(blocks, size=sample_size, replace=False)

        selected_normal_blocks = sample_blocks(unique_normal_blocks)
        selected_anomaly_blocks = sample_blocks(unique_anomaly_blocks)
        selected_blocks = np.concatenate([selected_normal_blocks, selected_anomaly_blocks])

        target_fraction_df = target_train_df[
            target_train_df["Node_block_id"].isin(selected_blocks)
        ].copy()

        print(f"[INFO] Target adaptation fraction per class: {fraction}")
        print(f"[INFO] Selected target normal blocks: {len(selected_normal_blocks)}")
        print(f"[INFO] Selected target anomaly blocks: {len(selected_anomaly_blocks)}")
        print(f"[INFO] Selected target rows: {len(target_fraction_df)}")

        return target_fraction_df
    ...
    print(f"[INFO] Target adaptation fraction per class: {TARGET_ADAPTATION_FRACTION}")
    ...
    # ------------------------------------------------------------
    # Select fraction of normal + anomaly target-train blocks
    # ------------------------------------------------------------

    target_fraction_df = select_target_fraction(
        target_train_df,
        TARGET_ADAPTATION_FRACTION,
        seed=SEED
    )

    target_fraction_df = target_fraction_df.copy()

    # Prevent Node_block_id collision between target-adaptation fraction and sources.
    target_fraction_df["Node_block_id"] = (
        TARGET_DATASET
        + "__target_train_adapt__"
        + target_fraction_df["Node_block_id"].astype(str)
    )
    ...
    train_df = pd.concat(
        [source_train_df, target_fraction_df],
        ignore_index=True
    )
